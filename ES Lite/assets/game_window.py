...
class Game:
    ...
    def update_game(self):
        ...
        if 0 <= self.second_time <= 180 and self.boss_fight == False:
            if len(self.enemies) <= 20:
                self.spawn_enemies_student(current_time)

        if self.second_time >= 120 and (self.boss_fight == False) and self.second_time <= 250:
            if len(self.enemies) <= 30:
                self.spawn_enemies_staff(current_time)

        if self.second_time >= 150 and self.boss_fight == False:
            if len(self.enemies) <= 30:
                self.spawn_enemies_guard(current_time)

        if self.second_time >= 220 and self.boss_fight == False:
    ...
    def level_up(self):
        ...
        self.player.neededXP += math.ceil(self.player.neededXP * 0.12)
        self.render_list = []
        self.cards = []
